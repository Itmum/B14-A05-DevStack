...
const TechCard = ({ tech }: techprops) => {
  const { name, category, description, icon, rating, difficulty, badge } = tech;
  return (
    <div className=" rounded-[24px] border border-slate-100 bg-white p-6 shadow-sm flex flex-col justify-between">
      ...
      <button className="mt-6 w-full py-3 bg-[#0B0F19] text-white font-semibold rounded-xl text-sm transition-all hover:bg-slate-800 active:scale-[0.98] shadow-sm">
        Add to Stack
      </button>
    </div>
  );
